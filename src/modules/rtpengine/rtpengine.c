...
static int ki_rtpengine_copy_offer(struct rtpengine_session *sess, str **to_tag,
		str *flags, unsigned int copy_flags, str *ret_body,
		struct rtpengine_streams *ret_streams);
static int ki_rtpengine_copy_answer(
		struct rtpengine_session *sess, str *to_tag, str *flags, str *body);
static int ki_rtpengine_copy_delete(
		struct rtpengine_session *sess, str *to_tag, str *flags);
static bencode_item_t *rtpengine_api_copy_op(struct rtpengine_session *sess,
...
static int bind_rtpengine(rtpengine_api_t *api)
{
	...
	api->rtpengine_copy_offer = ki_rtpengine_copy_offer;
	api->rtpengine_copy_answer = ki_rtpengine_copy_answer;
	api->rtpengine_copy_delete = ki_rtpengine_copy_delete;
		...
		struct rtpengine_streams *ret, struct rtpengine_session *sess)
{
	...
	for(item = streams->child; item; item = item->sibling) {
		...
		if(!tmp.s)
			...
			if(sess && sess->caller_tag && sess->caller_tag->len > 0
					&& tmp.len == sess->caller_tag->len
					&& memcmp(tmp.s, sess->caller_tag->s, tmp.len) == 0) {
				leg = RTPENGINE_CALLER;
			} else if(sess && sess->callee_tag && sess->callee_tag->len > 0
					  && !(tmp.len == sess->callee_tag->len
							  && memcmp(tmp.s, sess->callee_tag->s, tmp.len)
										 == 0)) {
				leg = RTPENGINE_CALLER;
			} else {
		...
		struct rtpengine_streams *ret_streams)
{
	str tmp;
	bencode_item_t *ret;
	ret = rtpengine_api_copy_op(
			sess, OP_SUBSCRIBE_REQUEST, *to_tag, flags, copy_flags, NULL);
	if(!ret)
		return -1;
...
{
	bencode_item_t *ret;
	ret = rtpengine_api_copy_op(sess, OP_UNSUBSCRIBE, to_tag, flags, 0, NULL);
	if(!ret)
		return -1;
	bencode_buffer_free(bencode_item_buffer(ret));
	return ret != NULL;
